...
const Navbar: FC<SidebarProps> = ({ setExpand }) => {
  ...
  const generateIcon = (icon: string) => {
    ...
    icons_map['perusahaan'] = (
      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-current" viewBox="0 0 24 24" fill="none">
        <path
    ...
    icons_map['mou'] = (
      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-current" viewBox="0 0 25 24" fill="none">
        <path
          opacity="0.48"
      ...
      <li key={index}>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className={`truncate ${isExpand ? '' : isExpandOnHover ? '' : 'w-0 h-0 opacity-0'}`}>{item.title}</div>
          </div>
          {'child' in item ? (
