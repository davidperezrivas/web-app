...
import { deleteInventoryElement } from '../services/purchase.service';
import { ModalDeleteProps } from '../models/Inventory';

const DeleteModalInventory = ({ closeEvent, id }: ModalDeleteProps) => {
  ...
  const deleteInventory = useMutation({
    mutationFn: deleteInventoryElement,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['getInventory'] });
      setToast({
        ...toast,
        show: true,
        error: 'Se ha eliminado ',
      });
